...
pub fn process_borrow(ctx: Context<Borrow>, amount: u64) -> Result<()> {
    ...
    let boorowable_amount = total_collateral.checked_mul(bank.liquidation_threshold).unwrap();

    if boorowable_amount < amount {
    ...
    msg!("LAST UPDATED BORRRRRRRRRROWWWWWWWWWWWWWWWWWWW: {}", user.last_updated_borrow);
...
pub fn calculate_accrued_interest(deposited_value: u64, interest_rate: f64, last_updated: i64) -> Result<u64> {
    let current_time = Clock::get()?.unix_timestamp;
    let time_diff = current_time - last_updated;
    let new_value = (deposited_value as f64 * E.powf(interest_rate * time_diff as f64)) as u64;
